    ...
    async with aiohttp.ClientSession(headers=HEADERS) as session:
        ...
        async with session.post(f"{BASE_URL}?action=login&t={int(time.time()*1000)}", data=login_payload) as resp:
            ...
            if login_json.get("status") != "success":
                print(f"[{acc_name}] Login rejected: {login_json}")
                return
        ...
        for task_id in shuffled_tasks:
            ...
            for attempt in range(3):
                try:
                    async with session.post(f"{BASE_URL}?action=start_task&t={int(time.time()*1000)}", data=payload) as resp:
        ...
        # 3. Dynamic Human-like Safe Wait Window (افزایش زمان به بازه ۳۲ تا ۳۷ ثانیه برای عبور قطعی از تایمر سرور)
        ...
        # 4. Claim Tasks (افزایش تلاش به ۵ بار با تاخیر هوشمند)
        for task_id in shuffled_tasks:
            ...
            for attempt in range(5):
                try:
                    async with session.post(f"{BASE_URL}?action=claim_task&t={int(time.time()*1000)}", data=payload) as resp:
                        ...
                        if '"status":"success"' in claim_res:
                            ...
                            if attempt == 4:
                                print(f"[{acc_name}] Claimed '{task_id}' failed after 5 retries -> {claim_res[:60]}")
                except Exception as ex:
                    if attempt == 4:
                        print(f"[{acc_name}] Network error on '{task_id}': {ex}")

                # در صورت ناموفق بودن، ۳ ثانیه مهلت برای پاسخ سرور
        ...
        for attempt in range(3):
            try:
                async with session.post(f"{BASE_URL}?action=claim&t={int(time.time()*1000)}", data=yellow_button_payload) as resp:
...
async def main():
    if not ACCOUNTS:
        print("No accounts found in ACCOUNTS_JSON environment variable.")
        return
